...
- Publication 后台详情页已优化为成果中枢，读取现有 Publication 字段、`project_id`、关联 Project、同项目 Knowledge、RelatedDocumentsPanel 和后台搜索入口；`file_path` 不展示也不作为下载入口，`cover_url` 仅作为安全 metadata 状态展示。当前不新增 Publication 到 Knowledge / Skill 的显式关系，不新增关系表、migration、RPC、AI、OCR、文件内容索引或向量搜索。
