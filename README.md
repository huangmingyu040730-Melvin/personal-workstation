...
- Resume 已接入履历素材库基础数据模型；管理员可在 `/dashboard/resume` 维护教育、实习、项目、研究、Skill、证书和奖项等结构化素材。
