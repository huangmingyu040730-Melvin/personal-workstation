...
当前项目状态详见 `docs/current-status.md`，公开内容运营指南详见 `docs/public-content-operations.md`，访问申请后台处理流程详见 `docs/access-request-workflow.md`，Access Grants 授权管理流程详见 `docs/access-grants-workflow.md`，viewer restricted 访问流程详见 `docs/viewer-restricted-access.md`，后续阶段规划详见 `docs/roadmap.md`，已知问题详见 `docs/known-issues.md`。
...
- Phase 2R-C-2 发布前 QA 与 hardening：新增轻量 `npm run smoke:public` 巡检公开路由、fallback、sitemap、robots、metadata 与敏感字段边界，并修补公开文案和附件 metadata 长内容换行；不新增 migration、不改 RLS / Storage / Documents / Access Grants
- Phase 2R-D-1 公开内容运营基础：Project / Publication / Knowledge / Skill 后台详情页新增基于既有字段的 public readiness checklist，并新增公开内容运营文档；只提示、不阻止保存、不自动公开内容或附件，不新增 migration、不改 RLS / Storage / Documents / public 下载 route / Access Grants
- Phase 2R-E-1 访问申请后台流程 polish：后台申请列表新增状态 / 目标类型筛选和更清晰的申请卡片，详情页升级为人工审核工作台；approved 仍只代表处理状态，不自动创建 Access Grant、不发邮件、不开放 Documents 或 signed URL
- Phase 2R-E-2 Access Grants 后台管理 polish：授权列表新增有效 / 过期 / 撤销和内容类型筛选，新增授权详情页，创建页和撤销区强化手动选择 restricted 内容与安全边界说明；不新增 migration、不改 RLS / Storage / Documents / public 下载 route / Access Grants 核心权限
- Phase 2I / Hotfix viewer restricted access：公开申请提交优先使用服务端安全写入，四类详情页切换到 RLS 保护的 viewable 查询，viewer magic link callback 成功后写入 session 并跳回安全 next；不开放 Documents、private attachments、zip、Storage path、Storage bucket 或 signed URL
...
- `/dashboard/access-grants/new` 手动创建 restricted 内容授权
- `/dashboard/access-grants/[id]` 单条访问授权详情与撤销
...
- Phase 2R-A-1 起公开首页和公开导航强调“黄铭语研究工作站”站点身份，首页展示研究方向、公开 Project / Publication / Knowledge / Skill 预览和访问申请入口；#100 追加 UI polish 将 hero H1 改为“个人研究工作站”，恢复左侧定位 / 标签 / CTA 与右侧统计卡片首屏结构，Knowledge / Skill 首页预览改为紧凑卡片并展示最多 4 条，补充克制的 hover / focus micro-interactions，并在公开导航加入轻量“管理员登录”入口。Phase 2R-A-2 在不改布局的前提下为 hero 增加金融 / 量化 / 研究风格的低对比 CSS 背景元素，并优化 H1 系统字体栈。Phase 2R-A-3 将四个公开列表页升级为统一 listing header、轻量筛选、公开内容卡片和友好空状态。Phase 2R-A-4A 允许公开 Project / Publication 详情页展示显式 public 文件附件；0021 hotfix 补齐该 server-side 查询所需的 `service_role` 只读授权。Phase 2R-A-4B 将公开 Project / Publication / Knowledge / Skill 详情页统一为正式研究详情体验、related public content 和安全 metadata。Phase 2R-B-1 将访问申请页、未公开内容 fallback 和后台申请审核体验 polish 为正式访客闭环：申请按钮带公开上下文，申请页展示目标与边界，后台能看到来源和目标。Phase 2R-C-1 统一公开 SEO、分享卡片、canonical、sitemap 和 robots：sitemap 只收录 public 内容，robots 阻止 dashboard、API、viewer、public-files 等路径。Phase 2R-C-2 新增公开发布前 smoke 与轻量 hardening，用脚本和浏览器冒烟复查公开路由、fallback、metadata、sitemap、robots、access-request query、敏感字段和 390px 移动端。Phase 2R-D-1 在四类后台详情页加入 public readiness checklist，并新增公开内容运营指南，帮助管理员判断哪些内容适合设为 public。Phase 2R-E-1 将后台访问申请列表和详情页 polish 为人工审核工作台，便于筛选、记录备注和跳转手动授权。Phase 2R-E-2 将 Access Grants 后台列表 / 新建 / 详情 / 撤销体验 polish 为手动授权工作台，支持有效 / 过期 / 撤销和内容类型筛选。公开页面仍只展示 public 内容，不公开 private Documents、Storage 路径、signed URL、`file_path`、raw link rows 或后台关系管理。
...
- 公开访客可以在 `/access-request` 提交访问申请；从 Project / Publication / Knowledge / Skill 公开详情页进入时，申请链接会带 `content_type`、slug、公开标题和来源。后台申请列表支持状态 / 目标类型筛选，详情页用于人工核对来源、理由、处理状态和内部备注。访问申请只是申请，不自动授权、不发送邮件、不开放 Documents 或 signed URL；管理员可在后台将指定 restricted 内容授权给指定邮箱。Viewer 登录和 restricted 只读访问基础代码已实现，但 magic link 登录仍存在已知问题，详见 `docs/known-issues.md`。
...
- Access Requests 使用真实 Supabase 表记录访问申请；匿名访客只能提交，管理员可按状态 / 目标类型筛选申请，查看来源、目标标题 / slug、理由摘要、完整理由和处理记录，并更新 pending / approved / rejected 状态与内部备注。
- Access Grants 已具备后台创建、列表、详情和撤销管理；列表支持有效 / 过期 / 撤销与内容类型筛选，创建仍必须手动选择具体 restricted 内容。Phase 2I / Hotfix 后，viewer 可通过 magic link 和 RLS 读取被授权的具体 restricted 内容；真实邮箱链路仍需在 Preview / 生产手动验收。
...
Phase 2I / Hotfix 已完成 viewer restricted access 的代码层面修复：授权邮箱通过 magic link 登录后，可由 RLS 读取被授权的具体 restricted 内容。真实 Supabase 邮件收取、callback 和四类 restricted 内容访问仍需在 Vercel Preview / 生产环境用测试邮箱人工验收。
