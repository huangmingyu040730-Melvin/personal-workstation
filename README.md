...
- Market Briefs 市场简报后台手工 CRUD、基于素材包的 AI-first 生成、每日市场素材包基础层、官方交易所总貌 summary facts、Markdown 主内容、站内预览图表、多格式下载、历史交易日补生成与生成任务记录
...
MARKET_BRIEF_AI_TIMEOUT_MS=105000
```

`MARKET_BRIEF_GENERATOR` 未配置时默认 `ai`；Phase 2N-B 后，后台生成按钮会先检查同日期同市场是否存在 `ready` / `partial` / `reviewed` 素材包，没有可用素材包时不会创建或继续执行 AI 生成任务，并提示先采集素材包。Phase 2N-C1 后，素材包手动采集会先读取上交所 / 深交所官方 summary / overview 公开接口，把交易所总貌字段写入 `extracted_facts.exchange_summary`，再通过 `MARKET_BRIEF_SEARCH_PROVIDER` 补充新闻 / 热点来源；Tavily / Serper / custom search 只作为 supplemental search，不作为行情事实来源。生成阶段不再默认实时搜索，只读取已保存素材包 sources / source notes / warnings / extracted facts，再复用 `AI_PROVIDER` / `AI_API_KEY` / `AI_BASE_URL` / `AI_MODEL` 或旧 `OPENAI_API_KEY` / `OPENAI_MODEL` 生成固定模板 Markdown、structured JSON、来源列表和预览图表。`MARKET_BRIEF_AI_TIMEOUT_MS` 可选，默认 105000，用于控制 AI 生成主动超时。`MARKET_BRIEF_SEARCH_PROVIDER` 支持 `disabled | tavily | serper | custom`；`mock` 仅用于本地占位测试。旧外部 / Python 数据 runner 的 API 与脚本已移除；AKShare / Eastmoney 不作为生产依赖；Market Brief 当前主路径是“每日市场研究素材包 -> AI 固定模板简报 -> 人工编辑确认”，不新增 cron。普通后台页面继续使用登录管理员身份与 RLS。
...
- `/dashboard/market-briefs/materials` 市场素材包列表与手动采集
- `/dashboard/market-briefs/materials/[id]` 市场素材包详情
...
- Phase 2L / 2N：Market Brief AI 生成、每日市场素材包基础层，以及后续 Notion / Google Calendar / AI 辅助研究。
...
- Market Briefs 已接入后台私密 `market_briefs` CRUD；管理员可在 `/dashboard/market-briefs` 手工维护每日市场收评，字段包括日期、标题、市场、状态、标签、数据来源和摘要 / 市场概览 / 指数表现 / 风格表现 / 行业板块 / 热点 / 资金流向 / 政策新闻 / 风险提示 / 明日关注等模块。Phase 2L-B 进一步增加 Markdown 主内容源、`/dashboard/market-briefs/[id]/preview` 站内预览、Markdown / HTML / JSON / Word 即时下载，以及浏览器打印 / 保存 PDF。Phase 2L-D-E 后，今日和历史生成都会在服务端校验 A 股交易日。Phase 2M-A / 2M-B 后，主生成链路收口为 AI-first；Phase 2M-C 增加 web search grounding；Phase 2M-D 增加任务进度体验：点击生成会创建 generation job 并进入 `/dashboard/market-briefs/jobs/[id]`，页面展示 AI 生成阶段、百分比和轮询状态，成功后自动跳转预览页。Hotfix 进一步增加默认 105 秒主动超时、5 分钟 stale running 检测、failed 落库保护、JSON code fence / loose JSON 解析、格式异常时保存待复核草稿、输出 token 限制和 sources / prompt 长度限制，避免任务卡在 writing / 70% 或因格式问题整单失败。Phase 2N-0 已移除旧外部 / Python runner 可执行路径；Phase 2N-A 新增后台私密 `market_brief_material_packages`，管理员可在 `/dashboard/market-briefs/materials` 手动采集指定日期和市场的公开来源素材包，并在详情页查看 queries、warnings、source notes、sources、source snapshot 和 extracted facts。Phase 2N-B 后，生成 job 默认读取已保存素材包，`request_payload.grounding_mode` 记录为 `material_package`，`source_snapshot.meta` 保存 material package id/status/collected_at；没有可用素材包时不调用搜索或 AI Provider，而是提示先采集素材包。Phase 2N-C1 后，手动采集会把上交所 / 深交所官方 summary 写入 `exchange_summary` 并在详情页展示“交易所总貌”卡片；由于 market breadth、行业板块和资金流仍缺，新采集素材包最多标记为 partial。partial 素材包允许生成，但输出会保留 needs_review / ai_grounded_partial 语义。本阶段不做完整自动行情采集、新闻爬虫、邮件发送、Notion 同步、定时任务、公开页面、股票推荐或投资建议。
...
- `market_brief_generation_jobs`：Phase 2L-D-A 至 2N-B 市场简报生成任务记录，保存 owner、日期、市场、任务状态、生成器名称（兼容字段 `runner_name`）、request payload、source snapshot、result payload、关联 `market_brief_id`、错误信息和运行时间线；`request_payload.is_historical` 标记历史补生成，`request_payload.progress` 保存 AI 生成阶段、百分比和提示文案，Phase 2N-B 起 `request_payload.grounding_mode` 和 `material_package_id` 标记素材包生成来源；AI 生成结果的 structured JSON 和 charts 保存在 `source_snapshot` / `result_payload` 中，queued / running 会阻止重复任务，failed / cancelled 可重新创建或重新排队；仅管理员或 owner 可通过后台读取管理，anon/viewer/public 不可访问。
- `market_brief_material_packages`：Phase 2N-A 每日市场素材包基础层，保存 owner、素材日期、市场、采集状态、搜索 provider、queries、sources、source snapshot、extracted facts、warnings、source notes、质量分、错误信息和采集 / 复核时间；通过 `(owner_id, package_date, market)` 避免重复包，仅管理员或 owner 可读写，anon/viewer/public 不可访问。Phase 2N-B 起，ready / partial / reviewed 素材包可驱动 AI 简报生成；collecting / failed / archived 不可用于生成。
