...
- Market Briefs 已接入后台私密 `market_briefs` CRUD；管理员可在 `/dashboard/market-briefs` 手工维护每日市场收评，字段包括日期、标题、市场、状态、标签、数据来源和摘要 / 市场概览 / 指数表现 / 风格表现 / 行业板块 / 热点 / 资金流向 / 政策新闻 / 风险提示 / 明日关注等模块。Phase 2L-B 进一步增加 Markdown 主内容源、`/dashboard/market-briefs/[id]/preview` 站内预览、Markdown / HTML / JSON / Word 即时下载，以及浏览器打印 / 保存 PDF。Phase 2L-D-E 后，今日和历史生成都会在服务端校验 A 股交易日。Phase 2M-A / 2M-B 后，主生成链路收口为 AI-first；Phase 2M-C 增加 web search grounding；Phase 2M-D 增加任务进度体验：点击“获取今日市场动态”或“生成指定日期市场简报”会创建 generation job 并进入 `/dashboard/market-briefs/jobs/[id]`，页面展示 AI 生成阶段、百分比和轮询状态，成功后自动跳转预览页。Hotfix 进一步增加 45 秒主动超时、stale running 检测、failed 落库保护、JSON code fence 解析和 sources / prompt 长度限制，避免任务卡在 writing / 70%。AI 生成会先检索公开市场来源，再调用 OpenAI-compatible AI Provider 生成固定模板 Markdown、structured JSON、`source_snapshot.sources` 和图表数据；预览页会展示正文、检索来源和图表，图表数据项必须带 `source_ids`。旧 external / Python 数据源 runner 已 deprecated，仅保留为历史诊断和兼容资料。本阶段不做行情接口抓取、新闻爬虫、邮件发送、Notion 同步、定时任务、公开页面、股票推荐或投资建议。
