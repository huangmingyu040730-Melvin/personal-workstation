...
- Market Briefs 市场简报后台手工 CRUD、基于素材包的 AI-first 生成、每日市场素材包基础层、Markdown 主内容、站内预览图表、多格式下载、历史交易日补生成与生成任务记录
...
`MARKET_BRIEF_GENERATOR` 未配置时默认 `ai`；Phase 2N-B 后，后台生成按钮会先检查同日期同市场是否存在 `ready` / `partial` / `reviewed` 素材包，没有可用素材包时不会创建或继续执行 AI 生成任务，并提示先采集素材包。素材包采集阶段通过 `MARKET_BRIEF_SEARCH_PROVIDER` 检索公开市场来源；生成阶段不再默认实时搜索，只读取已保存素材包 sources / source notes / warnings / extracted facts，再复用 `AI_PROVIDER` / `AI_API_KEY` / `AI_BASE_URL` / `AI_MODEL` 或旧 `OPENAI_API_KEY` / `OPENAI_MODEL` 生成固定模板 Markdown、structured JSON、来源列表和预览图表。`MARKET_BRIEF_AI_TIMEOUT_MS` 可选，默认 105000，用于控制 AI 生成主动超时。`MARKET_BRIEF_SEARCH_PROVIDER` 支持 `disabled | tavily | serper | custom`，推荐第一版使用 `tavily` 采集素材包。`mock` 仅用于本地占位测试。旧外部 / Python 数据 runner 的 API 与脚本已移除；Market Brief 当前主路径是“每日市场研究素材包 -> AI 固定模板简报 -> 人工编辑确认”，不新增 cron。普通后台页面继续使用登录管理员身份与 RLS。
