...
- Market Briefs 市场简报后台手工 CRUD、AI-first 生成、Markdown 主内容、站内预览图表、多格式下载、历史交易日补生成与生成任务记录
...
`MARKET_BRIEF_GENERATOR` 未配置时默认 `ai`；`ai` 模式会先通过 `MARKET_BRIEF_SEARCH_PROVIDER` 检索公开市场来源，再复用 `AI_PROVIDER` / `AI_API_KEY` / `AI_BASE_URL` / `AI_MODEL` 或旧 `OPENAI_API_KEY` / `OPENAI_MODEL`，在服务端生成固定模板 Markdown、structured JSON、来源列表和预览图表。后台按钮会先创建 generation job，再进入任务详情页展示 AI 生成进度动画；完成后自动跳转到市场简报预览页。`MARKET_BRIEF_AI_TIMEOUT_MS` 可选，默认 105000，用于控制 AI 生成主动超时。`MARKET_BRIEF_SEARCH_PROVIDER` 支持 `disabled | tavily | serper | custom`，推荐第一版使用 `tavily`。未配置搜索服务时，生成任务会失败并提示配置 `MARKET_BRIEF_SEARCH_PROVIDER` 和 `MARKET_BRIEF_SEARCH_API_KEY`，避免继续生成空模板。`mock` 仅用于本地占位测试。`MARKET_BRIEF_GENERATOR=external`、`MARKET_BRIEF_RUNNER_SECRET`、`MARKET_BRIEF_DATA_MODE` 和旧 external / Python 数据源 runner 已废弃，仅保留为历史兼容和诊断路径，不再是推荐市场简报生成方式。普通后台页面继续使用登录管理员身份与 RLS。
