...
- 后台全局搜索 `/dashboard/search`，按数据库 metadata 查找研究资产
...
- 后台全局搜索 `/dashboard/search` 只查询数据库 metadata，每类最多返回 8 条；不读取文件正文，不解析 PDF / Office / zip，不做 OCR、AI 摘要或向量搜索，不生成 signed URL，不输出 Storage path。
