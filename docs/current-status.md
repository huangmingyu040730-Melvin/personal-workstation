...
- 后台查看申请。
- 后台列表展示状态统计、状态筛选、目标类型筛选、申请人、邮箱、申请目标、来源、slug、公开路径、申请理由摘要、备注状态、状态和提交 / 处理时间。
- 后台详情升级为人工审核工作台，展示申请人信息、申请目标、来源、公开路径、原始申请链接、完整理由、处理记录、内部备注和安全边界。
- `pending` / `approved` / `rejected` 状态。
- 管理员备注。
- Dashboard 待处理申请提示。
- 已同意申请可继续跳转创建 Access Grant，但仍需管理员手动选择具体 restricted 内容；跳转只预填邮箱、申请 id 和内容类型，不自动选择 private id。

申请审批状态不等同于内容授权。访问授权通过 Access Grants 单独创建和撤销。2R-B-1 / 2R-E-1 不新增 migration、不新增邮件服务、不自动开放 Documents、private attachments、signed URL、public attachments 安全规则或 Storage 路径。
