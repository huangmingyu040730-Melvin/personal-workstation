...
- Profile 个人公开信息编辑基础。
- Calendar 站内日程 CRUD。
- Market Briefs 市场简报后台手工 CRUD、AI-first + web grounding 生成、任务进度动画、Markdown 主内容、站内来源 / 图表预览、多格式下载、历史交易日补生成与生成任务记录。
- Resume 履历素材库基础 CRUD。
- Resume 简历版本组合与后台预览。
...
Phase 2L-D-C 不新增 migration，曾新增 `scripts/market-brief-runner/python/` 作为旧数据源实验路径。Phase 2M-B 后，该 Python runner 仅保留为 deprecated 历史诊断与兼容资料，不再作为推荐生成方式。
...
- Phase 2L-D-A：Market Brief generation jobs / 生成任务、数据快照和私有结果回写接口。
- Phase 2L-D-B：External generation compatibility / external 模式、任务领取 / 失败回写 API 和脚本骨架。
- Phase 2L-D-C：Deprecated real-data runner experiment / 旧 Python 数据源实验、宽基指数、市场宽度、行业板块和 Markdown 生成。
...
- Phase 2M-B：Clean legacy market data runner path / 后台主流程与文档全面收口为 AI-first，旧 external / Python 数据源 runner 仅保留 deprecated 兼容与历史诊断说明。
...
Hotfix 修复生产环境中 Tavily / DeepSeek 已扣费但任务卡在 writing / 70% 的问题：`generate-ai` API 增加 45 秒主动超时并在超时或异常时安全标记 failed，status API 对 running 且超过 2 分钟未更新的任务返回 stale 提示，前端停止自动重复生成并提示管理员重置为排队后重试；同时限制搜索 query、source 数量、snippet 长度和 prompt 示例体积，并增强 AI JSON code fence 解析。
