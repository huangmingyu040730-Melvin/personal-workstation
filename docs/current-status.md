...
- Phase 2K-C：A4 中文简历模板化预览与浏览器打印 PDF。
- Phase 2K-D：AI JD 优化。

Phase 2K-A 当前新增 `resume_items` 数据模型和后台 `/dashboard/resume` 管理入口。Phase 2K-B 新增 `resume_versions`、`resume_version_items` 和 `/dashboard/resume/versions` 管理入口，用于组合素材、排序、分区和后台预览。Phase 2K-C 将预览页调整为接近传统中文金融简历的 A4 排版，并通过浏览器打印功能导出 PDF；本阶段不做后端 PDF、Word 导出、AI 生成简历、公开简历页面或分享链接。
