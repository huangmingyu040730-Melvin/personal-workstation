...
Phase 2L-D-C 不新增 migration，主要新增 `scripts/market-brief-runner/python/`，通过 AkShare 接入第一版真实 A 股基础市场数据。
