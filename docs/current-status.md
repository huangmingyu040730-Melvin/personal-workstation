...
- 研究资产显式关联关系 `research_asset_links`，用于在 Project / Knowledge / Skill / Publication 之间维护管理员手动确认的关系和 backlinks，并支持关系编辑、筛选统计、目标资产本地筛选和后台全局动态关系图谱。
...
- Phase 2Q-B-2 / 2Q-B-3 / 2Q-B-4 不新增 migration，不修改 `0019_research_asset_links.sql`，不新增 RPC，也不引入数据库事务。
- `/dashboard/network` 只读，不提供 create / edit / delete，不做批量关系管理。
- 不读取文件正文，不读取 Storage object，不生成 signed URL，不展示 Storage path。
- 不做 AI 自动关联、3D 图谱、拖拽连线、图谱编辑或复杂权限继承。
...
Phase 2Q-B-2 / 2Q-B-3 / 2Q-B-4 只优化显式关系管理体验和后台只读网络视图，不新增 migration，不修改已执行的 `0019_research_asset_links.sql`。
