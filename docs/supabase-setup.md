...
- Phase 2R-A-1 公开首页、公开导航和管理员登录入口 polish 不需要新增 migration；公开页面继续使用既有 public 查询和 RLS 边界，不公开 Documents、附件下载、Storage 路径、signed URL 或后台关系管理。管理员登录入口只进入现有登录流程，不改变 Auth、RLS 或后台权限。
