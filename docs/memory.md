...
- Phase 3A：Admin AI Content Copilot，四类后台详情页新增 AI 内容助手；Server Action 验证管理员后只读取安全字段，AI 输出只供人工参考，不保存数据库、不自动公开、不读取 Documents / Storage。
...
- Phase 3A 后，后台 AI 内容助手复用 `AI_PROVIDER` / `AI_API_KEY` / `AI_BASE_URL` / `AI_MODEL` 和旧 `OPENAI_API_KEY` / `OPENAI_MODEL`；未配置时安全降级，不影响后台页面渲染。
...
- Phase 3A 采用 admin-only AI copilot 决策：AI 只在后台帮助管理员整理公开摘要、标签、内容结构和风险提示；输入来自服务端白名单字段，不接受任意 prompt，不读 Documents / Storage，不保存输出，不修改资产或 visibility。
...
- Phase 3A：不新增 migration；只新增后台 AI 内容助手 Server Action、组件和文档，不修改 RLS、Storage policy、Documents、public 下载 route、公开页面或外部授权退役边界。
