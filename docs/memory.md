...
5. Workstation CLI 可继续小范围验证：先用静态 token 跑 health dataAccess、metadata list、Knowledge `--project-id` 过滤和 private create；下一步优先观察生产 / 本地 data access 配置、网络稳定性和错误提示，再单独评审 operation logs / permission hardening 或文件上传。
