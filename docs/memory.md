...
- v1.1.3 Resume export typography fixes：只修简历预览与 Word 导出的排版细节，统一邮箱字段与其他个人信息字段的字体 / 段落样式，并让实习经历的岗位 / 部门行加粗；不改变简历数据结构、照片上传 / 导出安全逻辑、数据库、Storage、RLS 或 public download route。
