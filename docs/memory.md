...
- v1.2.15 后，跨项目使用 Workstation 仍是设计态：当前 repo-level Skill 不等于全局 Skill。其他项目若要轻量接入，后续需要单独实现 Skill Pack / installer / standalone client；目标项目只应配置本地 `WORKSTATION_API_URL` / `WORKSTATION_API_TOKEN`，不得复制 `.env.local`、service role key 或真实 token 到仓库。
