...
- Phase 2R-F-2：公开首页精选内容 polish，首页精选区按 featured 优先、最近更新补足的规则展示 public Projects / Publications / Knowledge / Skills，并新增轻量“关于这个工作站”section；不新增 migration、不改公开查询权限、不恢复外部访问链路。
...
- Phase 2R-F-2 后，公开首页精选区只使用 public 查询返回的既有字段；排序展示在首页层完成，先取 `is_featured = true` 内容，不足时用最近更新的 public 记录补足。
...
- Phase 2R-F-2 采用 public homepage featured content polish 决策：首页精选内容只做展示层整理，优先 featured public 内容并以最近更新 public 内容补足；空状态进入对应公开列表页，不引入访问申请、Viewer、Access Grants、Documents 入口或新的授权流程。
...
- Phase 2R-F-2：不新增 migration；只 polish 首页精选内容展示、About section 和 public smoke 首页断言，不修改 RLS、Storage policy、Documents、public 下载 route 或四类内容 schema。
