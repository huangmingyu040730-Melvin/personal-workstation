...
- v1.2.8 后，Workstation document upload 仍是设计阶段，不可调用真实上传命令。未来实现必须走 `upload-intent -> controlled upload -> finalize`，只允许上传单个本地文件到已有 document collection，默认 private，由服务端生成 ASCII-safe Storage path，并写 operation logs 安全摘要；CLI 不得生成 Storage path、读取 Documents 正文、创建 collection、公开发布、修改 visibility、生成 public link 或持有 service role key。
