...
- Phase 2K-C：将后台预览优化为 A4 中文简历模板，并支持浏览器打印 / 另存为 PDF。
- Phase 2K-D：AI 辅助 JD 优化。

Phase 2K-A 只建立数据模型与后台素材 CRUD，不做 PDF 导出、Word 导出、AI 生成、模板系统、公开简历页或英文简历。Phase 2K-B 增加简历版本和素材选择关系，支持按区块排序、展示开关、版本语言/模板标记和后台预览。Phase 2K-C 采用浏览器打印导出 PDF，不引入后端 PDF 服务、Word 导出、公开简历页面或 AI JD 优化；更复杂模板和 Word 导出继续后延。
