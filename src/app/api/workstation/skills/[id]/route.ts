...
import { summarizeShowRequest, summarizeSkillUpdateRequest } from "@/lib/workstation/request-summary";
import { getWorkstationLookupType, showWorkstationSkill, updateWorkstationSkill } from "@/lib/workstation/query";
import { workstationSkillUpdateSchema } from "@/lib/workstation/schemas";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const context = createWorkstationRequestContext(request, "skills.show", "skill");
  ...
  try {
    ...
    const result = await showWorkstationSkill(lookup);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const context = createWorkstationRequestContext(request, "skills.update", "skill");
  ...
  try {
    ...
    const requestSummary = summarizeSkillUpdateRequest(id, bodyResult.body);
    const parsed = workstationSkillUpdateSchema.safeParse(bodyResult.body);

    if (!parsed.success) {
      return finishWorkstationError(context, {
        code: "VALIDATION_ERROR",
        message: parsed.error.issues[0]?.message ?? "Invalid skill update payload.",
        status: 400,
        requestSummary,
        targetId: id
      });
    ...
    const result = await updateWorkstationSkill(id, parsed.data);
