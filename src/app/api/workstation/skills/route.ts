...
import { summarizeListRequest, summarizeSkillCreateRequest } from "@/lib/workstation/request-summary";
import {
...
export async function GET(request: NextRequest) {
  const context = createWorkstationRequestContext(request, "skills.list", "skill");
...
export async function POST(request: Request) {
  const context = createWorkstationRequestContext(request, "skills.create", "skill");
  ...
  try {
    ...
    const requestSummary = summarizeSkillCreateRequest(bodyResult.body);
    const parsed = workstationSkillCreateSchema.safeParse(bodyResult.body);

    if (!parsed.success) {
      return finishWorkstationError(context, {
        code: "VALIDATION_ERROR",
        message: parsed.error.issues[0]?.message ?? "Invalid skill payload.",
        status: 400,
        requestSummary
      });
    }
