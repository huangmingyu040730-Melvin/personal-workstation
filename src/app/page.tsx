...
export default async function HomePage() {
  ...
  return (
    <PublicShell>
      <section className="public-soft-grid relative overflow-hidden border-b border-slate-200">
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-slate-100/70 to-transparent" />
        <div className="public-reveal-slow relative z-10 mx-auto grid max-w-[1680px] gap-12 px-5 py-16 lg:grid-cols-[1.08fr_0.92fr] lg:px-12 lg:py-24 2xl:px-16">
          <div className="flex flex-col justify-center">
            <Badge className="mb-5 w-fit bg-blue-50 text-blue-700 ring-blue-100">公开研究工作站</Badge>
            <h1 className="public-display max-w-5xl text-6xl font-semibold leading-tight tracking-normal text-navy-950 md:text-7xl">
              {profile.name}
            </h1>
            <p className="mt-5 text-xl font-semibold text-blue-700">投资研究、量化分析与 AI 工作流探索者</p>
            ...
            <div className="mt-7 flex flex-wrap gap-2">
              {["私募基金研究", "量化策略分析", "AI 辅助研究", "数据分析", "知识管理", "自动化工作流"].map((interest) => (
                <span key={interest} className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-sm text-slate-700 shadow-sm">
                  {interest}
                </span>
              ))}
            </div>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <Link href="/publications" className="public-cta-motion inline-flex items-center justify-center rounded-2xl border border-blue-100 bg-blue-50 px-5 py-3 text-sm font-semibold text-blue-700 transition hover:border-blue-200 hover:bg-blue-100">
                查看学术成果
              </Link>
              <Link href="/skills" className="public-cta-motion inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:text-blue-700">
                进入 Skill 库
              </Link>
            </div>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
            {[
              ...
              <Link key={item.title} href={item.href} className="group block">
                <Card className="public-card-motion min-h-44 border-slate-200 bg-white p-6 shadow-soft">
                  <item.icon className="text-blue-700 transition duration-300 group-hover:scale-110 group-hover:text-blue-600" size={28} />
                  <p className="mt-8 text-4xl font-semibold text-navy-950">{item.value}</p>
                  <div className="mt-1 flex items-center justify-between gap-3 text-sm">
                    <span className="text-slate-600">{item.title}</span>
                    <span className="inline-flex items-center gap-1 text-xs font-semibold text-blue-700 opacity-80">
      ...
      <section className="mx-auto grid max-w-[1680px] gap-6 px-5 pb-14 lg:grid-cols-[1fr_0.42fr] lg:px-12 2xl:px-16">
        ...
        <Card className="border-slate-200 bg-white shadow-soft">
          ...
          <div className="space-y-4 text-sm leading-7 text-slate-600">
            ...
            <Link href="/access-request" className="public-cta-motion inline-flex w-fit items-center gap-2 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-100">
              申请查看受限内容
              <ArrowRight size={16} />
            </Link>
          </div>
      ...
      <section className="border-t border-slate-200 bg-white">
        <div className="mx-auto grid max-w-[1680px] gap-5 px-5 py-11 md:grid-cols-[1fr_auto] md:items-center lg:px-12 2xl:px-16">
          <div>
            <p className="text-sm font-semibold uppercase text-blue-700">About</p>
            <h2 className="public-display mt-2 text-2xl font-semibold text-navy-950">了解这个公开研究工作站</h2>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              这个网站对外展示公开研究，对内承载私密数字资产后台。你可以从这里进入研究项目、学术成果、Skill 和知识文章，也可以了解我的研究方向与后续规划。
            </p>
          </div>
          <Link href="/about" className="public-cta-motion inline-flex items-center justify-center gap-2 rounded-2xl bg-navy-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-800">
            <UserRound size={18} />
            关于我
          </Link>
        </div>
