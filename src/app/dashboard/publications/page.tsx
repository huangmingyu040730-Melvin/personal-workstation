      ...
      <Card>
        ...
        {publications.length === 0 ? (
          <div className="rounded-2xl bg-slate-50 p-6 text-center">
            ...
            <Link href="/dashboard/publications/new" className="mt-4 inline-flex rounded-2xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white">新建成果</Link>
          </div>
        ) : (
          <div className="space-y-3">
            {publications.map((publication) => (
              <Link key={publication.id} href={`/dashboard/publications/${publication.id}`} className="flex items-center justify-between gap-4 rounded-2xl bg-slate-50 p-4 transition hover:bg-blue-50">
