...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (error || !document) {
    return NextResponse.redirect(new URL(`/dashboard/documents/${id}?error=${encodeURIComponent("文件不存在或当前账号无权下载。")}`, _request.url));
  }

  const { data, error: signedUrlError } = await createDocumentSignedUrl(supabase, document.storage_path);

  if (signedUrlError || !data?.signedUrl) {
    return NextResponse.redirect(new URL(`/dashboard/documents/${id}?error=${encodeURIComponent("生成临时下载链接失败，请稍后重试。")}`, _request.url));
  }
