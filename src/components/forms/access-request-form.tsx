...
export function AccessRequestForm({ error, success }: { error?: string; success?: boolean }) {
  if (success) {
    return (
      <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6">
        <p className="text-lg font-semibold text-emerald-900">申请已提交</p>
        <p className="mt-2 text-sm leading-7 text-emerald-800">申请已提交，我会在合适的时候查看并处理。</p>
    ...
    <form action={submitAccessRequestAction} className="space-y-5">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Field label="希望查看的内容类型">
          <Select name="requested_content_type" defaultValue="">
        ...
        <Field label="希望查看的内容标题">
          <TextInput name="requested_content_title" maxLength={160} />
        </Field>
      </div>
      <Field label="内容链接" hint="可选。可以填写站内路径，例如 /projects/example，或公开网页链接。">
        <TextInput name="requested_content_url" maxLength={300} />
      </Field>
      <Field label="申请理由" hint="请简要说明希望查看的内容和用途。不要填写敏感个人隐私或机密信息。">
      ...
      <div className="rounded-2xl bg-blue-50 px-4 py-3 text-sm leading-7 text-blue-800">
        提交申请不代表一定会授权。本阶段仅记录访问申请与处理状态，不会自动开放任何受限内容。
      </div>
