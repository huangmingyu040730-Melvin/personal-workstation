...
function TagList({ tags, tone = "slate", limit = 4 }: { tags: string[]; tone?: "slate" | "blue" | "green"; limit?: number }) {
  ...
  const toneClass = tone === "blue" ? "bg-blue-50 text-blue-700" : tone === "green" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600";
...
export function PublicProjectCard({ project }: { project: ProjectRecord }) {
  return (
    <Link href={`/projects/${project.slug}`} className="group block h-full">
      <Card className="public-card-motion public-reveal flex h-full flex-col border-slate-200 bg-white shadow-soft hover:border-blue-200">
        ...
        <div className="mt-auto flex items-center justify-between border-t border-slate-100 pt-4">
          ...
          <span className="inline-flex items-center gap-1 text-sm font-semibold text-blue-700">详情 <ArrowRight className="transition group-hover:translate-x-1" size={15} /></span>
        </div>
      </Card>
    </Link>
  );
...
export function PublicSkillCard({ skill }: { skill: SkillRecord }) {
  return (
    <Link href={`/skills/${skill.slug}`} className="group block h-full">
      <Card className="public-card-motion public-reveal relative flex h-full flex-col overflow-hidden border-slate-200 bg-white shadow-soft hover:border-blue-200">
        <div className="absolute right-0 top-0 h-24 w-24 rounded-bl-full bg-blue-100/70 transition duration-300 group-hover:scale-125" />
...
export function PublicKnowledgeCard({ note }: { note: KnowledgeNoteRecord }) {
  return (
    <Link href={`/knowledge/${note.slug}`} className="group block h-full">
      <Card className="public-card-motion public-reveal flex h-full flex-col border-slate-200 bg-white shadow-soft hover:border-blue-200">
        <div className="flex gap-3">
          <BookOpen className="mt-1 shrink-0 text-blue-700" size={18} />
          <div className="min-w-0">
