    ...
    <section className="relative overflow-hidden border-b border-slate-200 bg-white">
      ...
      <div className="public-reveal relative mx-auto grid max-w-[1680px] gap-8 px-5 py-10 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.42fr)] lg:items-end lg:px-12 lg:py-14 2xl:px-16">
        ...
        <div className="grid grid-cols-2 gap-3">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-2xl border border-white/80 bg-white/88 p-4 shadow-[0_14px_40px_rgba(15,42,88,0.08)] backdrop-blur">
              <p className="text-2xl font-semibold text-navy-950">{stat.value}</p>
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((option) => (
          <Link
            ...
            href={option.href}
            className={cn(
              "rounded-full border px-3 py-1.5 text-xs font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2",
