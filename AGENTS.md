...
- `/dashboard/network` 只读展示 `research_asset_links` 的全局研究资产关系图谱，不提供 create / edit / delete；2Q-B-4 起使用 `react-force-graph-2d` 提供动态 2D force graph，并在首屏展示动态图谱主卡片、视觉增强和节点局部 focus mode；不直接引入 d3、cytoscape、react-flow、three.js 或 3D 图谱库，不纳入 Documents，不读取 Storage 或文件正文。
